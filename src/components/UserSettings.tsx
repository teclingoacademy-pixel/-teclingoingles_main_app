  ...
  const cargarPerfilDesdeLake = async () => {
    ...
    if (effectiveRole === 'ALUMNO') {
      ...
      setInstData(prev => ({
        ...
        // Leer turnos activos desde columnas booleanas
        turnos: [
          perfil.turno_matutino === 'TRUE' ? 'MATUTINO' : null,
          perfil.turno_vespertino === 'TRUE' ? 'VESPERTINO' : null,
          perfil.turno_semi_escolarizado === 'TRUE' ? 'SEMI-ESCOLARIZADO' : null,
          perfil.turno_sabatino === 'TRUE' ? 'SABATINO' : null,
          perfil.turno_distancia === 'TRUE' ? 'DISTANCIA / EN LÍNEA' : null,
             ...
             <p className="text-white/20 text-[6px] sm:text-[8px] font-black uppercase tracking-widest">
               ID: {effectiveRole === 'ALUMNO'
                 ...
                 : effectiveRole === 'DIRECTOR'
                   ...
                   : (teacherData.employeeId || 'usr_xxx')}
             </p>
           </div>
        </div>
                             ...
                             <div className="space-y-2 sm:space-y-3 mb-4 sm:mb-6">
                                <div className="flex items-center justify-between">
                                   ...
                                   <button
                                    type="button"
                                    onClick={() => {
                                      const nueva = prompt('Nombre de la carrera (se guardará en MAYÚSCULAS):');
                                      if (nueva && nueva.trim()) {
                                        const normalized = nueva.trim().toUpperCase();
                                    ...
                                    className="flex items-center gap-1 px-2 sm:px-3 py-1 sm:py-1.5 bg-[#DEFF9A]/10 border border-[#DEFF9A]/20 rounded-lg text-[8px] sm:text-[9px] font-black uppercase tracking-widest text-[#DEFF9A] hover:bg-[#DEFF9A]/20 transition-all"
                                   >
                             ...
                             <div className="space-y-2 sm:space-y-3">
                                <div className="flex items-center justify-between">
                                   ...
                                   <button
                                    type="button"
                                    onClick={() => {
                                      const nuevo = prompt('Nombre del turno (se guardará en MAYÚSCULAS):');
                                      if (nuevo && nuevo.trim()) {
                        ...
                        <div className="space-y-1.5 sm:space-y-2">
                         ...
                         {effectiveRole === 'ALUMNO' && (
                           <>
                             ...
                             <div className="space-y-1.5 sm:space-y-2">
                                <label className="text-[8px] sm:text-[9px] font-black text-white/20 uppercase tracking-widest ml-1">Carrera</label>
                                <select
                             ...
                             <div className="space-y-1.5 sm:space-y-2">
                                <label className="text-[8px] sm:text-[9px] font-black text-white/20 uppercase tracking-widest ml-1">Turno</label>
                                <select
                                 value={profile.shift || ''}
...
{effectiveRole === 'DOCENTE' && (
                            <div className="space-y-1.5 sm:space-y-2">
                               ...
                               <input
                                ...
                                className="w-full bg-white/5 border border-white/10 rounded-xl sm:rounded-2xl py-2.5 sm:py-4 px-3 sm:px-6 text-[#DEFF9A] text-xs font-black outline-none focus:border-[#38BDF8]/40 transition-all uppercase"
                               />
                            </div>
                          )}
                        ...
                        {effectiveRole === 'ALUMNO' && (
                          <div className="md:col-span-2 mt-4 sm:mt-8 p-4 sm:p-8 rounded-2xl sm:rounded-[3rem] bg-gradient-to-br from-[#DEFF9A]/10 to-transparent border border-[#DEFF9A]/20">
                             ...
                             <div className="flex items-center gap-4 sm:gap-8 bg-black/40 p-3 sm:p-6 rounded-xl sm:rounded-[2rem] border border-white/5 relative overflow-hidden group">
                                <div className="w-14 h-14 sm:w-20 sm:h-20 rounded-xl sm:rounded-2xl border border-white/10 overflow-hidden shrink-0">
                                ...
                                <div className="flex-1 min-w-0">
                                   ...
                                   <div className="flex flex-wrap items-center gap-1.5 sm:gap-3">
                                      <span className="text-white/40 text-[8px] sm:text-[10px] font-mono tracking-widest uppercase">ID: {teacherData.employeeId}</span>
